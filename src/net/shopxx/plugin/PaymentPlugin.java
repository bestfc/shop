...
public abstract class PaymentPlugin implements Comparable<PaymentPlugin> {
	...
	//定义全局的plugin的ID
	public static final String ID_AlipayAppPayment = "alipayAppPayment";
	public static final String ID_WechatAppPayment = "wechatAppPayment";
	 ...
	 * @return
	 */
	public String getPostrefundUrl(PaymentPlugin paymentPlugin, PaymentTransaction paymentTransaction){
		return getPostrefundUrl(paymentPlugin, paymentTransaction, null);
	}
	/**
	 * 获取退款后处理URL
	 *
	 ...
	 * @return 支付后处理URL
	 */
	public String getPostrefundUrl(PaymentPlugin paymentPlugin, PaymentTransaction paymentTransaction, String extra) {
		Assert.notNull(paymentPlugin);
		Assert.hasText(paymentPlugin.getId());
		Assert.notNull(paymentTransaction);
		Assert.hasText(paymentTransaction.getSn());
		...
		Setting setting = SystemUtils.getSetting();
		return setting.getSiteUrl() + "/payment/post_refund_" + paymentTransaction.getSn() + (StringUtils.isNotEmpty(extra) ? "_" + extra : "");
	}
	...
	//提供取消订单的接口
	public abstract boolean cancel(String outTradeNo);

	//提供退款的接口
	public abstract boolean refund(PaymentTransaction paymentTransaction, String refundSn, BigDecimal refundAmount,String refundReason);
