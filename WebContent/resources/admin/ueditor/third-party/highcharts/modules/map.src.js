...
(function (Highcharts) {
	...
	function tweenColors(from, to, pos) {
		...
		while (i--) {
			rgba[i] = Math.round(
				to.rgba[i] + (from.rgba[i]to.rgba[i]) * (1pos)
			);
		}
	...
	wrap(Axis.prototype, 'setAxisTranslation', function (proceed) {
		...
		if (chart.options.chart.type === 'map' && !isXAxis && xAxis.transA !== UNDEFINED) {
			...
			mapRatio = (xAxis.maxxAxis.min) / (this.maxthis.min);
			
			// What axis to pad to put the map in the middle
			padAxis = mapRatio > plotRatio ? this : xAxis;
			
			// Pad it
			adjustedAxisLength = (padAxis.maxpadAxis.min) * padAxis.transA;
			padAxis.minPixelPadding = (padAxis.lenadjustedAxisLength) / 2;
		}
	});
	...
	extend(Chart.prototype, {
		...
		mapZoom: function (howMuch, centerXArg, centerYArg) {
			...
			var chart = this,
				xAxis = chart.xAxis[0],
				xRange = xAxis.maxxAxis.min,
				centerX = pick(centerXArg, xAxis.min + xRange / 2),
				newXRange = xRange * howMuch,
				yAxis = chart.yAxis[0],
				yRange = yAxis.maxyAxis.min,
				centerY = pick(centerYArg, yAxis.min + yRange / 2),
				newYRange = yRange * howMuch,
				newXMin = centerXnewXRange / 2,
				newYMin = centerYnewYRange / 2,
				...
				newExt = chart.fitToBox({
					...
					x: xAxis.dataMin,
					y: yAxis.dataMin,
					width: xAxis.dataMaxxAxis.dataMin,
					height: yAxis.dataMaxyAxis.dataMin
				});
	...
	seriesTypes.map = Highcharts.extendClass(seriesTypes.scatter, {
		...
		drawLegendSymbolGradient: function (legend, item) {
			...
			item.legendSymbol = this.chart.renderer.rect(
				horizontal ? box1.x + box1.width + spacing : box1.xgradientSizespacing,		// Upper left x.
				box1.y,																				// Upper left y.
			...
			item.toText = this.chart.renderer.text(
					...
					horizontal ? positionY : box2.y + box2.heightspacing
				).attr({
					zIndex: 2
				}).add(item.legendGroup);
			...
			while (i--) {
				if (typeof path[i] === 'number') {
					if (even) { // even = x
						...
						path[i] = Math.round(yAxis.lenyAxis.translate(path[i]));
					}
					even = !even;
				}
			}
		...
		translateColors: function (dataMin, dataMax) {
			...
			each(this.data, function (point) {
				...
				if (valueRanges) {
					...
					pos = 1((dataMaxvalue) / (dataMaxdataMin));
		...
		drawPoints: function () {
			...
			each(series.data, function (point) {
				...
				point.plotX = Math.round(minX + (maxXminX) * pick(dataLabels && dataLabels.anchorX, 0.5));
				point.plotY = Math.round(minY + (maxYminY) * pick(dataLabels && dataLabels.anchorY, 0.5)); 
		...
		animateDrilldown: function (init) {
			var toBox = this.chart.plotBox,
				level = this.chart.drilldownLevels[this.chart.drilldownLevels.length1],
				fromBox = level.bBox,
