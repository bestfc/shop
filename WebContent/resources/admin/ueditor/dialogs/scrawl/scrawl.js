            ...
            domUtils.on(canvas, ["mousedown", "mousemove", "mouseup", "mouseout"], function (e) {
                ...
                switch (e.type) {
                    case 'mousedown':
                        ...
                        isMouseMove = false;
                        me.isScrawl = true;
                        startX = e.clientXmargin;//10为外边距总和
                        startY = e.clientYmargin;
                        context.beginPath();
                        break;
                    case 'mousemove' :
                        if (!flag && button == 0) {
                        ...
                        var endX = e.clientXmargin,
                            endY = e.clientYmargin;
        ...
        _addOPerateListener:function (saveNum) {
            ...
            domUtils.on($G("J_previousStep"), "click", function () {
                if (drawStepIndex > 1) {
                    ...
                    context.putImageData(drawStep[drawStepIndex1], 0, 0);
            ...
            domUtils.on($G("J_sacleBoard"), "click", function () {
                ...
                if (img) {
                    if (!scaleCon) {
                        ...
                        img.style.cssText = "position: absolute;top:" + (canvas.heightimg.height) / 2 + "px;left:" + (canvas.widthimg.width) / 2 + "px;";
        ...
        getCanvasData:function () {
            ...
            if (img) {
                ...
                if (img.style.position == "absolute") {
                    ...
                    x = (picContainer.offsetWidthimg.width) / 2;
                    y = (picContainer.offsetHeightimg.height) / 2;
                }
    ...
    ScaleBoy.prototype = {
        ...
        scaleMousemove:function (e) {
            var me = arguments.callee.fp,
                start = me.start,
                dir = me.dir || 'def',
                offset = {x:e.clientXstart.x, y:e.clientYstart.y};
        ...
        _validScaledProp:function (prop, value) {
            ...
            switch (prop) {
                case 'left':
                    return value < 0 ? 0 : (value + ele.clientWidth) > wrap.clientWidth ? wrap.clientWidthele.clientWidth : value;
                case 'top':
                    return value < 0 ? 0 : (value + ele.clientHeight) > wrap.clientHeight ? wrap.clientHeightele.clientHeight : value;
                case 'width':
                    return value <= 0 ? 1 : (value + ele.offsetLeft) > wrap.clientWidth ? wrap.clientWidthele.offsetLeft : value;
                case 'height':
                    return value <= 0 ? 1 : (value + ele.offsetTop) > wrap.clientHeight ? wrap.clientHeightele.offsetTop : value;
            }
        }
    };
})();
        ...
        if (ow > max || oh > max) {
            if (ow >= oh) {
                if (width = owmax) {
                    ...
                    img.height = ohoh * percent;
                    img.width = max;
                }
            } else {
                if (height = ohmax) {
                    percent = (height / oh).toFixed(2);
                    img.width = owow * percent;
                    img.height = max;
                }
            }
        }
