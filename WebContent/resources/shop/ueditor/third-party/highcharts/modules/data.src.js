				...
				if (rowNo >= startRow && rowNo <= endRow && !isComment && !isBlank) {
					...
					each(items, function (item, colNo) {
						if (colNo >= startColumn && colNo <= endColumn) {
							if (!columns[colNostartColumn]) {
								columns[colNostartColumn] = [];
							}
							
							columns[colNostartColumn][activeRowNo] = item;
						}
					});
					activeRowNo += 1;
				}
				...
				if (rowNo >= startRow && rowNo <= endRow) {
					each(tr.childNodes, function (item) {
						if ((item.tagName === 'TD' || item.tagName === 'TH') && colNo >= startColumn && colNo <= endColumn) {
							...
							columns[colNo][rowNostartRow] = item.innerHTML;
			...
			jQuery.getJSON('https://spreadsheets.google.com/feeds/cells/' + 
				...
				for (i = 0; i < colCount; i++) {
					if (i >= startColumn && i <= endColumn) {
						...
						columns[istartColumn] = [];

						// Setting the length to avoid jslint warning
						columns[istartColumn].length = Math.min(rowCount, endRowstartRow);
					}
				}
				...
				for (i = 0; i < cellCount; i++) {
					cell = cells[i];
					gr = cell.gs$cell.row1; // rows start at 1
					gc = cell.gs$cell.col1; // columns start at 1

					// If both row and col falls inside start and end
					// set the transposed cell value in the newly created columns
					if (gc >= startColumn && gc <= endColumn &&
						gr >= startRow && gr <= endRow) {
						columns[gcstartColumn][grstartRow] = cell.content.$t;
					}
				}
				self.dataFound();
			});
