...
/**
 * @license canvg.jsJavascript SVG parser and renderer on Canvas
 * MIT Licensed 
 * Gabe Lerner (gabelerner@gmail.com)
 * http://code.google.com/p/canvg/
 *
 * Requires: rgbcolor.jshttp://www.phpied.com/rgb-color-parser-in-javascript/
 *
 */
if(!window.console) {
	window.console = {};
	...
	function build() {
		...
		svg.init = function(ctx) {
			...
			svg.ViewPort = new (function () {
				...
				this.Current = function() { return this.viewPorts[this.viewPorts.length1]; }
		...
		svg.Point = function(x, y) {
			...
			this.angleTo = function(p) {
				return Math.atan2(p.ythis.y, p.xthis.x);
			}
		...
		svg.BoundingBox = function(x1, y1, x2, y2) { // pass in initial points if you want
			...
			this.width = function() { return this.x2this.x1; }
			this.height = function() { return this.y2this.y1; }
			...
			this.addQuadraticCurve = function(p0x, p0y, p1x, p1y, p2x, p2y) {
				var cp1x = p0x + 2/3 * (p1xp0x); // CP1 = QP0 + 2/3 *(QP1-QP0)
				var cp1y = p0y + 2/3 * (p1yp0y); // CP1 = QP0 + 2/3 *(QP1-QP0)
				var cp2x = cp1x + 1/3 * (p2xp0x); // CP2 = CP1 + 1/3 *(QP2-QP0)
				var cp2y = cp1y + 1/3 * (p2yp0y); // CP2 = CP1 + 1/3 *(QP2-QP0)
		...
		svg.AspectRatio = function(ctx, aspectRatio, width, desiredWidth, height, desiredHeight, minX, minY, refX, refY) {
			// aspect ratiohttp://www.w3.org/TR/SVG/coords.html#PreserveAspectRatioAttribute
			...
			if (refX.hasValue() && refY.hasValue()) {				
				...
				// align
				if (align.match(/^xMid/) && ((meetOrSlice == 'meet' && scaleMin == scaleY) || (meetOrSlice == 'slice' && scaleMax == scaleY))) ctx.translate(width / 2.0desiredWidth / 2.0, 0); 
				if (align.match(/YMid$/) && ((meetOrSlice == 'meet' && scaleMin == scaleX) || (meetOrSlice == 'slice' && scaleMax == scaleX))) ctx.translate(0, height / 2.0desiredHeight / 2.0); 
				if (align.match(/^xMax/) && ((meetOrSlice == 'meet' && scaleMin == scaleY) || (meetOrSlice == 'slice' && scaleMax == scaleY))) ctx.translate(widthdesiredWidth, 0); 
				if (align.match(/YMax$/) && ((meetOrSlice == 'meet' && scaleMin == scaleX) || (meetOrSlice == 'slice' && scaleMax == scaleX))) ctx.translate(0, heightdesiredHeight); 
			}
		...
		svg.Element.rect = function(node) {
			...
			this.path = function(ctx) {
				...
				if (ctx != null) {
					ctx.beginPath();
					ctx.moveTo(x + rx, y);
					ctx.lineTo(x + widthrx, y);
					ctx.quadraticCurveTo(x + width, y, x + width, y + ry)
					ctx.lineTo(x + width, y + heightry);
					ctx.quadraticCurveTo(x + width, y + height, x + widthrx, y + height)
					ctx.lineTo(x + rx, y + height);
					ctx.quadraticCurveTo(x, y + height, x, y + heightry)
					ctx.lineTo(x, y + ry);
		...
		svg.Element.circle = function(node) {
			...
			this.path = function(ctx) {
				...
				return new svg.BoundingBox(cxr, cyr, cx + r, cy + r);
			}
		}
		...
		svg.Element.ellipse = function(node) {
			...
			this.path = function(ctx) {
				...
				if (ctx != null) {
					ctx.beginPath();
					ctx.moveTo(cx, cyry);
					ctx.bezierCurveTo(cx + (KAPPA * rx), cyry,  cx + rx, cy(KAPPA * ry), cx + rx, cy);
					ctx.bezierCurveTo(cx + rx, cy + (KAPPA * ry), cx + (KAPPA * rx), cy + ry, cx, cy + ry);
					ctx.bezierCurveTo(cx(KAPPA * rx), cy + ry, cxrx, cy + (KAPPA * ry), cxrx, cy);
					ctx.bezierCurveTo(cxrx, cy(KAPPA * ry), cx(KAPPA * rx), cyry, cx, cyry);
					ctx.closePath();
				}
				
				return new svg.BoundingBox(cxrx, cyry, cx + rx, cy + ry);
			}
		}
		...
		svg.Element.polyline = function(node) {
			...
			this.getMarkers = function() {
				var markers = [];
				for (var i=0; i<this.points.length1; i++) {
		...
		svg.Element.path = function(node) {
			...
			this.PathParser = new (function(d) {
				...
				this.isEnd = function() {
					return this.i >= this.tokens.length1;
				}
				...
				this.getReflectedControlPoint = function() {
					...
					// reflect point
					var p = new svg.Point(2 * this.current.xthis.control.x, 2 * this.current.ythis.control.y);					
					return p;
				}
					...
					case 'A':
						while (!pp.isCommandOrEnd()) {
							...
							var currp = new svg.Point(
								Math.cos(xAxisRotation) * (curr.xcp.x) / 2.0 + Math.sin(xAxisRotation) * (curr.ycp.y) / 2.0,
								-Math.sin(xAxisRotation) * (curr.xcp.x) / 2.0 + Math.cos(xAxisRotation) * (curr.ycp.y) / 2.0
							);
							// adjust radii
							...
							var centp = new svg.Point(
								(curr.x + cp.x) / 2.0 + Math.cos(xAxisRotation) * cpp.xMath.sin(xAxisRotation) * cpp.y,
							...
							if (sweepFlag == 0 && ad > 0) ad = ad2 * Math.PI;
							if (sweepFlag == 1 && ad < 0) ad = ad + 2 * Math.PI;

							// for markers
							var halfWay = new svg.Point(
								centp.xrx * Math.cos((a1 + ad) / 2),
								centp.yry * Math.sin((a1 + ad) / 2)
							);
							...
							if (ctx != null) {
								...
								ctx.scale(sx, sy);
								ctx.arc(0, 0, r, a1, a1 + ad, 1sweepFlag);
								ctx.scale(1/sx, 1/sy);
								ctx.rotate(-xAxisRotation);
								ctx.translate(-centp.x, -centp.y);
							}
		...
		svg.Element.AnimateBase = function(node) {
			...
			this.progress = function() {
				return ((this.durationthis.begin) / (this.maxDurationthis.begin));
			}			
		}
		...
		svg.Element.animate = function(node) {
			...
			this.calcValue = function() {
				...
				return from + (tofrom) * this.progress(); 
			};
		}
		...
		svg.Element.animateColor = function(node) {
			...
			this.calcValue = function() {
				...
				if (from.ok && to.ok) {
					...
					var r = from.r + (to.rfrom.r) * this.progress();
					var g = from.g + (to.gfrom.g) * this.progress();
					var b = from.b + (to.bfrom.b) * this.progress();
		...
		svg.Element.a = function(node) {
			...
			this.renderChildren = function(ctx) {
				if (this.hasText) {
					...
					var fontSize = new svg.Property('fontSize', svg.Font.Parse(svg.ctx.font).fontSize);
					svg.Mouse.checkBoundingBox(this, new svg.BoundingBox(this.x, this.yfontSize.Length.toPixels('y'), this.x + this.measureText(ctx), this.y));					
				}
				else {
					...
					for (var j=0; j<cssClasses.length; j++) {
						...
						if (cssClass != '') {
							...
							for (var k=0; k<cssProps.length; k++) {
								...
								var value = cssProps[k].substr(prop + 1, cssProps[k].lengthprop);
							...
							if (cssClass == '@font-face') {
								...
								for (var s=0; s<srcs.length; s++) {
									if (srcs[s].indexOf('format("svg")') > 0) {
										...
										var url = srcs[s].substr(urlStart + 5, urlEndurlStart6);
		...
		svg.Element.filter = function(node) {
			...
			this.apply = function(ctx, element) {
				...
				// render on me
				ctx.drawImage(c, 0, 0, width + 2*px, height + 2*py, xpx, ypy, width + 2*px, height + 2*py);
	...
	extend(CanVGRenderer.prototype, {
		...
		color: function (color, elem, prop) {
			if (color && color.linearGradient) {
				...
				color = color.stops[color.stops.length1][1];
			}
